...
def test_pipeline_produces_expected_white_box_funnel(tmp_path):
    ...
    assert len(workspace.batches) == 6
...
def test_agent_fixture_contains_turns_tools_and_steering(tmp_path):
    ...
    rows = [row for row in rows if row["metadata"].get("origin") == "codex"]
...
def test_pipeline_accepts_generic_records_without_origin_metadata(tmp_path):
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    record = {
        "record_id": "rec_custom_001",
        "dataset_id": "workflow-signals",
        "batch_id": "batch_custom_001",
        ...
        "payload": {
            ...
            "content": (
                "User stopped a running refactor and asked to preserve the public API contract."
            ),
            "status": "open",
        },
        "metadata": {"evidence_count": 3},
    }
    (raw_dir / "batch_custom_001.jsonl").write_text(json.dumps(record) + "\n")

    workspace = run_pipeline(raw_dir, tmp_path / "artifacts")

    assert workspace.dataset.record_count == 1
    assert workspace.batches[0].id == "batch_custom_001"
    assert workspace.records[0].batch_id == "batch_custom_001"
